...
const DoubtPage = () => {
    ...
    const getDoubts=async()=>{
        try{
            await axios.get(`http://localhost:5000/api/doubts/subject/${loc.state.detail.id}`).then((response)=>{
    ...
    const createDoubt=async(newDoubt)=>{
        try{
            ...
            return await axios.post(`http://localhost:5000/api/doubts`, newDoubt) .then((res)=> {
                toast.success('Posted Doubt sucessfully!', {position : toast.POSITION.TOP_RIGHT})
            });
    ...
    return (
        <div >
            ...
            <CardContent>
                ...
                <Box style={{margin:10}}>
                <Button variant="outlined" onClick ={() => postDoubt()}>Post doubt</Button>
                </Box>
            </CardContent>
            ...
            <div>
                {doubtData.map((doubt) => (
                    ...
                    <DoubtCard getDoubts={getDoubts} key = {doubt.id} doubts = {doubt}  >
